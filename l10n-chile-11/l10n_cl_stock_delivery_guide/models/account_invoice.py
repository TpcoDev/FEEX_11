    ...
    def _get_pending_pickings(self):
        for inv in self:
            ...
            if inv.type in ['out_invoice']:
                ...
                tz_current = (tz.localize(datetime.strptime(fecha_inicio, DTF)).astimezone(
                    pytz.utc) + relativedelta.relativedelta(months=mes_antes))
        ...
        for inv in self:
            ...
            if inv.move_id:
                for ref in inv.reference_ids:
                    if ref.reference_doc_type.code in [56]:
                        sp = self.env['stock_picking'].search([('sii_document_number', '=', ref.origin)])
                if sp:
                    if inv.type in ['out_invoice']:
                        sp.invoiced = True
                    else:
