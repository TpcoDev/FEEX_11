...
class LibroGuia(models.Model):
    ...
    @staticmethod
    def create_template_envio(RutEmisor, PeriodoTributario, FchResol, NroResol, EnvioDTE, signature_d,
                              TipoLibro='MENSUAL',TipoEnvio='TOTAL',FolioNotificacion="123", IdEnvio='SetDoc'):
        if TipoLibro in ['ESPECIAL']:
            FolioNotificacion = '''<FolioNotificacion>
    {0}
</FolioNotificacion>'''.format(FolioNotificacion)
        else:
            FolioNotificacion = ''
        xml = '''<EnvioLibro ID="{9}">
    <Caratula>
...
'''.format(seed)
    ...
    @staticmethod
    def create_template_env(doc, simplificado=False):
...
version="1.0">
    ...
    def getResumen(self, rec):
        ...
        det['RUTDoc'] = self.format_rut(rec.partner_id.vat or self.company_id.partner_id.main_id_number)
        ...
        if referencia:
            det['TpoDocRef'] = referencia.journal_document_class_id.code
            det['FolioDocRef'] = referencia.origin
            det['FchDocRef'] = referencia.date[:10]
        if rec.reference_ids:
            for r in rec.reference_ids:  # reparar para que no sobreescriba
                det['TpoDocRef'] = r.reference_doc_type.code
                det['FolioDocRef'] = r.origin
    ...
    def _crear_libro(self):
        ...
        dte['item'] = resumenes
        dte['TmstFirma'] = pysiidte.time_stamp()
        ...
        xml = pysiidte.create_template_env(libro)
        root = etree.XML(xml)
        envio_dte = etree.tostring(root, pretty_print=True).decode().replace(
    ...
    def _get_send_status(self, track_id, signature_d,token):
        ...
        if resp['SII:RESPUESTA']['SII:RESP_HDR']['ESTADO'] == "-11":
            status = {'warning': {'title': _('Error -11'), 'message': _(
                "Error -11: Espere a que sea aceptado por el SII, intente en 5s más")}}
    ...
    def _get_dte_status(self, signature_d, token):
        ...
        _server = Client(url, ns)
        receptor = self.format_rut(self.partner_id.vat or self.company_id.partner_id.main_id_number)
        date_invoice = datetime.strptime(self.date_invoice, "%Y-%m-%d").strftime("%d-%m-%Y")
        response = None
        retry = 0
        while response is None and retry < 1000:
            try:
                response = _server.getEstDte(signature_d['subject_serial_number'][:8], str(
                    signature_d['subject_serial_number'][-1]),
                        self.company_id.vat[2:-1],self.company_id.vat[-1], receptor[:8],receptor[2:-1],str(self.document_class_id.code), str(self.sii_document_number),
