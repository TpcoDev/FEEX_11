...
class DeliveryGuildeChangeCurrency(models.TransientModel):
    _name = 'delivery_guide.change_currency'
    ...
    currency_id = fields.Many2one(
        ...
        help="Select a currency to apply on the delivery guide"
    )
    ...
    def get_picking(self):
        ...
        if not picking:
            raise ValidationError(_('No Delivery Guide on context as "active_id"'))
        return picking
    ...
    def change_currency(self, ):
        ...
        if self.currency_rate >= 1:
            ...
            #   for line in picking.move_line_tax_ids.filtered(lambda x: x.manual):
        ...
        try:
            picking.comment = picking.comment[:picking.comment.find('//') + 1] + message2
