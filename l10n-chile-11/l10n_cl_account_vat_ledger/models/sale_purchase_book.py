...
class AccountMoveBook(models.Model):
    ...
    @staticmethod
    def month_delta(date_param, delta):
        m, y = (date_param.month + delta) % 12, date_param.year + (date_param.month + delta - 1) // 12
        if not m:
            m = 12
        d = min(date_param.day, [
            31, 29 if y % 4 == 0 and not y % 400 == 0 else 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][m - 1])
        return date_param.replace(day=d, month=m, year=y)
    ...
    state = fields.Selection([
        ...
        help="""
Statuses related to book sending to SII
        """)

    state_inner = fields.Selection([
        ('draft', 'Draft'),
        ('open', 'Open'),
        ('paid', 'Paid'), ], 'Inner State', readonly=True, default='draft',
        track_visibility='onchange', copy=False,
        help="""'Draft': unconfirmed book.
'Open': Confirmed.
'Paid': Transfered amount to SII (not implemented yet).""")
    
    journal_ids = fields.Many2many(
        'account.journal', string='Journals', readonly=True, states={'draft': [('readonly', False)]})
...
requiere un Código de Autorización de Reemplazo de Libro Electrónico.""")
    ...
    sending_type = fields.Selection([
        ...
        ('partial', 'Parcial'), 
        ('total', 'Total'),
        ('final', 'Final'), ], string="Tipo de Envío", default="total", required=True, readonly=True,
        states={'draft': [('readonly', False)], })
    ...
    fiscal_period = fields.Char(
        ...
        default=lambda self: self.month_delta(date.today(), -1).strftime('%Y-%m'),
        states={'draft': [('readonly', False)], })
    ...
    dte_service_provider = fields.Selection([
        ...
        'DTE Service Provider', readonly=True, related='company_id.dte_service_provider')

    @api.onchange('operation_type')
    def _onchange_optype(self):
        if not self.operation_type == 'purchase':
            self.is_fee = False
...
replace(main_id_number, '.', '') as "RUTDoc",
left(rp.name, 50) as "RznSoc",
(CASE WHEN ref.code IN ('29', '30', '32', '33', '34', '35', '38', '39', '40', '41', '43', '45',
'46', '48', '53', '55', '56', '60', '61', '101', '102', '103', '104', '105', '106', '108', '109', '110',
'111', '112', '175', '180', '185', '900', '901', '902', '903', '904', '905', '906', '907', '909', '910',
'911', '914', '918', '919', '920', '921', '922', '924', '500', '501')
THEN ref.code 
ELSE '' END) as "TpoDocRef",
(CASE WHEN ref.code IN ('29', '30', '32', '33', '34', '35', '38', '39', '40', '41', '43', '45',
'46', '48', '53', '55', '56', '60', '61', '101', '102', '103', '104', '105', '106', '108', '109', '110',
'111', '112', '175', '180', '185', '900', '901', '902', '903', '904', '905', '906', '907', '909', '910',
'911', '914', '918', '919', '920', '921', '922', '924', '500', '501')
THEN ref.origin 
ELSE '' END) as "FolioDocRef",
at.tax_group_id,
at.no_rec,
at.sii_code as tax_code,
ai.vat_for_common_use,
ai.no_rec_code,
al.price_subtotal_signed,
(al.price_subtotal_signed * at.amount / 100) as tax_amount,
ai.amount_untaxed,
ai.amount_total
...
{}</LibroCompraVenta>""".format(xml_pret)
    ...
    def _record_detail(self, summary, details):
        if True:  # try:
            ...
            dict1n = self.insert_son_values(
                summary, 'ResumenPeriodo', 'TotIVANoRec',
            ...
            xml_detail1 = self.replace_tags(self.replace_tags(
                self.replace_tags(
                    dicttoxml.dicttoxml(
                        ...
                        '.0<', '<'), tag_replace01, '0'), tag_replace_1, ''), tag_replace01, '0.0')
            dict2n = self.insert_son_values(
                details, 'Detalles', 'IVANoRec', ['CodIVANoRec', 'MntIVANoRec'])
            ...
            # reference_type_docs_included = ['29', '30', '32', '33', '34', '35', '38', '39', '40', '41', '43', '45',
            # '46', '48', '53', '55', '56', '60', '61', '101', '102', '103', '104', '105', '106', '108', '109', '110',
            # '111', '112', '175', '180', '185', '900', '901', '902', '903', '904', '905', '906', '907', '909', '910',
            # '911', '914', '918', '919', '920', '921', '922', '924', '500', '501']
            # todo: exclude more
...
<CodAutRec>{}</CodAutRec>\
                ...
                self.name.replace(' ', '_'),
                inv_obj.format_vat(self.company_id.main_id_number.replace('.', '')),
                signature_d.get('subject_serial_number',''),
                self.fiscal_period,
...
guardada del xml es la siguiente: {}'.format(xml_pret))
            ...
            certp = signature_d.get('cert','').replace(
                BC, '').replace(EC, '').replace('\n', '')
            xml_pret = self._envelope_book(xml_pret)
            type = 'bookbh' if self.is_fee else 'book'
            if self.dte_service_provider in ['SIIHOMO','SII']:
                xml_pret = self.sign_full_xml(
                    xml_pret.replace('<?xml version="1.0" encoding="ISO-8859-1"?>\n', ''),
                    signature_d.get('priv_key',''),
                    certp,
                    self.name.replace(' ', '_'),
                    type=type)
            else:
                xml_pret = xml_pret.replace('<?xml version="1.0" ?>', '')
                _logger.info("\n\n\n XML_PRET: %s \n\n\n" % xml_pret)
    ...
    def sign_full_xml(self, message, privkey, cert, uri, type='book'):
        ...
        signed_info_c14n = etree.tostring(
            signed_info, method="c14n", exclusive=False, with_comments=False, inclusive_ns_prefixes=None)
        ...
        modulus.text = textwrap.fill(base64.b64encode(self.long_to_bytes(
    ...
    def get_invoices(self):
        if len(self.journal_ids) == 0:
            raise UserError('You must select a mimimum of 1 %s journal.' % self.operation_type)
        
        domain = [('journal_id', 'in', self.journal_ids.ids),
                  ('state', 'not in', ['draft', 'cancel']),
                  ('date', '=like', self.fiscal_period + '%')]

        fee_doc_types = [self.env.ref('l10n_cl_account.dc_m_f_dtn').id,
                       self.env.ref('l10n_cl_account.dc_m_d_dtn').id]

        if self.is_fee:
            domain.append(('journal_document_type_id.document_type_id',
                       'in', fee_doc_types))
        elif self.operation_type == 'purchase':
            domain.append(('journal_document_type_id.document_type_id',
                       'not in', fee_doc_types))

        self.invoice_ids = self.invoice_ids.search(domain)

    @api.depends(
        'name', 'date', 'company_id', 'journal_ids','invoice_ids', 'fiscal_period',
        'operation_type', 'report_type', 'sending_type', 'proportion_factor')
    def set_values(self):
        for rec in self:
            self.get_invoices()
            if not self.invoice_ids.ids:
                self.name = "Periodo: %s Sin Facturas" % self.fiscal_period
                self.sii_xml_request = ''
                return
            dict0 = rec._summary_by_period()
            rec._record_totals(dict0)
            summary = {'ResumenPeriodo': dict0}
            details = {'Detalles': rec._detail_by_period()}
            xml_pret = rec._record_detail(summary, details)
            rec.sii_xml_request = xml_pret

    @api.multi
    def validate_report(self):
        for record in self:
            if record.state_inner not in ['draft']:
                raise UserError(u'The % book is in state: %s.' % (record.name, record.state_inner))
            record.state_inner = 'open'
            record.state = 'NoEnviado'

    @api.multi
    def change_to_draft(self):
        for record in self:
            if record.state_inner in ['draft']:
                raise UserError(u'The % book is in state: %s.' % (record.name, record.state_inner))
            record.state_inner = 'draft'
            record.state = 'draft'

    @api.multi
    def validate_send_report(self):
        for record in self:
            if record.state not in ['draft', 'NoEnviado', 'Rechazado']:
                raise UserError('El libro se encuentra en estado: {}'.format(
                    record.state))
            company_id = record.company_id
            doc_id = record.operation_type + '_' + record.fiscal_period
            result = record.send_xml_file(
                record.sii_xml_request, doc_id + '.xml', company_id)
            record.write({
                'sii_xml_response': result['sii_xml_response'],
                'sii_send_ident': result['sii_send_ident'],
                'state': result['sii_result'], }, )
        self.validate_report(self)
...
'''.format(seed)
    ...
    @api.depends('sii_xml_request')
    def get_values_from_xml(self):
        xml_dict = xmltodict.parse(self.sii_xml_request)
        
        summary = xml_dict['LibroCompraVenta']['EnvioLibro']['ResumenPeriodo']['TotalesPeriodo']
        summary_keys_ordered = [
        ...
        summary_totals = collections.OrderedDict()
        summary_list2 = []
        for value in summary:
            summary_list1 = []
            for key in summary_keys_ordered:
                try:
                    summary_totals[key] += float(value.get(key, 0))
                except KeyError:
                    summary_totals[key] = float(value.get(key, 0))

                summary_list1.append((key, value.get(key, '')))
            summary_list2.append(summary_list1)
        
        details = xml_dict['LibroCompraVenta']['EnvioLibro']['Detalle']
        details_keys_ordered = [
                'TpoDoc', 'NroDoc', 'FchDoc', 'RUTDoc', 'RznSoc',
                'MntExe', 'MntNeto', 'TasaImp', 'MntIVA', 'IVAUsoComun', 'MntSinCred', 'MntTotal']

        titles = {
            'fees':     [
                'Tipo', 'Folio', 'Fecha', 'RUT Prov', 'Nombre', '', 'Bruto', 'Tasa', 'Retención', '',
                '', 'Total'],
            'invoices': [
                'Tipo', 'Folio', 'Fecha', 'RUT Prov', 'Razon Soc', 'Monto Exento', 'Neto',  'Tasa', 'Impuesto',
                'Iva Uso Com', 'Monto Sin Cr.', 'Liquido'],
        }

        if isinstance(details, dict):
            details = [details]

        detail_totals = collections.OrderedDict()
        details_list2 = []
        for value in details:
            details_list1 = []
            for key in details_keys_ordered:
                try:
                    detail_totals[key] += float(value.get(key, 0))
                except KeyError:
                    try:
                        detail_totals[key] = float(value.get(key, 0))
                    except:
                        detail_totals[key] = 0.0
                except ValueError:
                    detail_totals[key] = 0.0
                except TypeError:
                    detail_totals[key] += 0.0

                details_list1.append((key, value.get(key, '')))
            details_list2.append(details_list1)

        values = {'summary_keys': summary_keys_ordered,
                  'summary': summary_list2,
                  'summary_totals': summary_totals,
                  'details_keys': details_keys_ordered,
                  'details': details_list2,
