    ...
    def generate_xlsx_report(workbook, data, records):
        ...
        for obj in records:
            ...
            for l in obj.line_ids:
                ...
                t_credit += l.credit
            row += 1
            sheet.write(row, 0, 'Total Asiento %s' % obj.name, entry_format)
            sheet.write(row, 1, '', entry_format)
            sheet.write(row, 2, '', entry_format)
            sheet.write(row, 3, '', entry_format)
            sheet.write(row, 4, t_debit, entry_format)
            sheet.write(row, 5, t_credit, entry_format)
