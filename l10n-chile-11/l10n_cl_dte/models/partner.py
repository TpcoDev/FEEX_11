...
class DteEmail(models.Model):
    ...
    pointless_main_id_number = fields.Char(
        'Pointless Main ID Number', compute='_pointless_main_id_number', store=True, readonly=True)

    @api.depends('main_id_number')
    def _pointless_main_id_number(self):
        for record in self:
            try:
                record.pointless_main_id_number = record.main_id_number.replace('.', '')
            except AttributeError:
                _logger.info('main_id_number es false, no se pudo actualizar campo pointless_main_id_number')
